...
class AnalystInbox:
    ...
    async def handle(self, message: AgentMessage) -> None:
        ...
        # ClarificationTask per question. Phase 4.5 (task-driven): the
        # tool-picker decides which tool to apply to each task; the
        # validator decides which task(s) a response actually
        # resolves. Dev-agent does NOT get ``plan.ready`` until human
        # answers come back and Analyst re-plans.
        if (
            ...
            if task_row is not None and plan_row_id is not None:
                try:
                    await self._tasks.request_clarifications(
                        task_row=task_row, plan=plan, plan_row_id=plan_row_id,
                    )
                except Exception:
                    logger.exception(
                        "AnalystInbox: task dispatch failed for {}",
                        external_id,
                    )
            return
