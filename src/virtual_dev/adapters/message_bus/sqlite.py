...
class SqliteMessageBus(MessageBusPort):
    ...
    async def _register_subscriber(self, agent_key: str) -> None:
        ...
        try:
            ...
            async with self._session_factory() as session:
                stmt = sqlite_insert(BusSubscriptionRow).values(
                    agent_key=agent_key,
                    registered_at=now,
                    last_seen_at=now,
                )
            ...
            logger.exception(
                "SqliteMessageBus: failed to register subscriber {!r}", agent_key,
            )

    async def _all_subscribers(self) -> list[str]:
        async with self._session_factory() as session:
            rows = (
                await session.execute(select(BusSubscriptionRow.agent_key))
            ).scalars().all()
        return list(rows)
