...
* Если мыслей больше одной (вывод + пара вопросов) — разбей на короткие
  абзацы пустой строкой; 2-3 вопроса можно оформить пунктами. Сплошное
  полотно читается хуже двух коротких абзацев. Структура ≠ длина:
  лимит выше это не отменяет.
* По-прежнему никаких markdown-заголовков и разбора вариантов «с
  критериями» в DM — структура только для читаемости, не для объёма.
...
Тоже хорошо (мыслей больше — абзац на вывод, абзац на вопросы):
«Привет! По DM-2740 предлагаю: падение критично, только если упал
структурный шаг или успешных источников < 95%.

В тикете "процент удаления" — это доля источников или объём удалённого?
И сайленс снимаем после выката?»
